...
import { loadScheduleMonth, saveScheduleMonth, type ScheduleDay, type DayType } from "./actions";
...
export default function ScheduleClient({ specialists }: { specialists: SpecOption[] }) {
  ...
  // панель времени — применяется к дням, которые делаем рабочими
  ...
  const load = useCallback(async () => {
    if (!specId) return;
    setLoading(true);
    const r = await loadScheduleMonth(specId, from, to);
    if (r.ok) {
  ...
  function cycle(dateStr: string) {
    setDays((prev) => {
      ...
      if (cur === null) {
        // серый → зелёный (рабочий, с текущими настройками панели)
        next[dateStr] = {
          type: "work",
          start,
          end,
  ...
  // применить пресет: сделать рабочими дни по маске недели
  ...
  // применить текущее время панели ко всем уже рабочим дням
  ...
  function save() {
    ...
    startSaving(async () => {
      const r = await saveScheduleMonth(specId, from, to, payload);
      if (r.ok) {
  ...
  return (
    <div className="mx-auto max-w-5xl px-8 py-8">
      ...
      <p className="mt-1 text-sm text-neutral-500">
        Задайте время работы, затем отмечайте дни в календаре. Клик переключает: не задан → рабочий → выходной.
      </p>
      ...
      <div className="mt-6 flex flex-wrap gap-2">
        {specialists.map((s) => (
          <button
            key={s.id}
            onClick={() => {
              if (dirty && !confirm("Есть несохранённые изменения. Сменить мастера?")) return;
              setSpecId(s.id);
            }}
      ...
      <div className="mt-4 rounded-xl border border-neutral-200 bg-white p-4">
        <div className="flex flex-wrap items-end gap-4">
          ...
          >
            Применить время ко всем рабочим дням
          </button>
        </div>
